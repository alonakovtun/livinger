...
var lastUnitNumber = 0;
var lastUnitNumberCategory = 0;
function galleryInit() {
  ...
  esGalleryFilterBtn.click(function () {
    lastUnitNumberCategory = 0;
    lastUnitNumber = 4;
    // var unitNumber = document.querySelectorAll('.unit-info__number');
    // unitNumber.forEach((item, index) => {
    //   lastUnitNumber = index + 1;
    //   console.log(lastUnitNumber);
    // });
...
function scrollDown(button) {
  if ($window.width() > 720) {
    if (button.textContent == 'info') {
      button.textContent = 'close info';
      var boxInfo = button.parentNode.parentNode.parentNode;
      boxInfo.classList.add('is-active');
    }
  }
}
function calcScroll() {
...
function attrVideo(videoElement) {
...
function closeProject(closeButton, activeClass) {
  var parentCloseElement = closeButton.parentNode.parentNode.parentNode;
  var videoElem = parentCloseElement.querySelector('video');

  jQuery('body')[0].style.overflow = 'auto';
  jQuery('body').css({ marginRight: '0px' });
  parentCloseElement.classList.remove(activeClass);
...
function hideShowHeader() {
  ...
  document.querySelector('#masthead').classList.add('loaded'),
    (i = document.querySelector('#masthead')),
    (r = window.pageYOffset),
    window.addEventListener('scroll', function () {
      var e = window.pageYOffset;
      var heightHeader = jQuery('.header__fake').height();
      var heightEgoFilm = jQuery('.egofilm-box').height();
      console.log(heightHeader + heightEgoFilm);
      heightHeader + heightEgoFilm <= e &&
        (e <= r ? i.classList.remove('hidden') : i.classList.add('hidden'),
        (r = e));
    });
}

function playPauseVideoHandler(button, video) {
  if (!video.paused) {
...
function fullScreenVideo(video, button) {
...
function checkSource(source, playButton, muteButton, fullButton) {
  if (!source) {
...
function muteUnmuteVideoHandler(button, video) {
  if (!video.muted) {
...
function infoCloseInfoHandler(button, openElement, activeClass) {
  if (button.textContent == 'info') {
    button.textContent = 'close info';
    openElement.classList.add(activeClass);
  } else {
...
function checkProductIndex(newProductLength) {
  ...
  newElements.forEach((element) => {
    var unitNumber = element.querySelectorAll('.unit-info__number');
    unitNumber.forEach((item) => {
      item.textContent = `NO.${
        lastUnitNumber < 9 ? `o${lastUnitNumber + 1}` : lastUnitNumber + 1
      }`;
      lastUnitNumber = lastUnitNumber + 1;
    });
    var containerElement = element.querySelector('.gallery__container');
    var videoElement = containerElement.previousElementSibling.querySelector(
      '.video-banner__video'
    );
    ...
    containerElement.addEventListener('click', function (e) {
      ...
      var boxInfo = infoCloseInfoButton.parentNode.parentNode.parentNode;
      if (infoCloseInfoButton.textContent === 'close info') {
        infoCloseInfoButton.textContent = 'info';
        boxInfo.classList.remove('is-active');
      }
    ...
    document.addEventListener('wheel', function (e) {
      if (e.deltaY > 0) {
        scrollDown(infoCloseInfoButton);
      }
    });
  });
  /* ==============================
...
function checkProductCategody() {
  var allProductArray = document.querySelectorAll('.project_unit');
  allProductArray.forEach((element) => {
    var unitNumber = element.querySelectorAll('.unit-info__number');
    unitNumber.forEach((item) => {
      item.textContent = `NO.${
        lastUnitNumberCategory < 9
          ? `o${lastUnitNumberCategory + 1}`
          : lastUnitNumberCategory + 1
      }`;
      lastUnitNumberCategory = lastUnitNumberCategory + 1;
    });
    var containerElement = element.querySelector('.gallery__container');
    var videoElement = containerElement.previousElementSibling.querySelector(
      '.video-banner__video'
...
jQuery(document).ready(function () {
  if (
    ...
    $('.gallery__container').on('mouseenter', function () {
      var videoContainerElement = $(this)[0].querySelector('video');

      videoContainerElement.removeAttribute('autoplay');
      videoContainerElement.setAttribute('muted', 'muted');
      videoContainerElement.muted = true;

      var playPromise = videoContainerElement.play();
      if (playPromise !== undefined) {
        playPromise
          .then((_) => {
            videoContainerElement.setAttribute('autoplay', 'autoplay');
            videoContainerElement.play();
          })
          .catch((error) => {
            console.log(error);
          });
    ...
    $('.gallery__container').on('mouseleave', function () {
      var videoContainerElement = $(this)[0].querySelector('video');
      var playPromise = videoContainerElement.pause();

      if (playPromise !== undefined) {
        playPromise
          .then((_) => {
            videoContainerElement.removeAttribute('autoplay');
            videoContainerElement.pause();
          })
          .catch((error) => {
            console.log(error);
          });
    ...
    function openProductHandler() {
      ...
      const muteButtons = document.querySelectorAll('.muted-li'),
        pauseButtons = document.querySelectorAll('.pause-li'),
        fullscreenButtons = document.querySelectorAll('.fullscreen-li'),
        openInfoButtons = document.querySelectorAll('.info'),
        closeButtons = document.querySelectorAll('.close-li');

      galleryContainers.forEach((galleryContainer) => {
        galleryContainer.addEventListener('click', function (e) {
          e.preventDefault();

          openInfoButtons.forEach((closeOpenInfoButton) => {
            var boxInfo = closeOpenInfoButton.parentNode.parentNode.parentNode;
            if (closeOpenInfoButton.textContent === 'close info') {
              closeOpenInfoButton.textContent = 'info';
              boxInfo.classList.remove('is-active');
            }
          });

          if (
            (window.innerWidth || document.documentElement.clientWidth) > 850
          ) {
            jQuery('.header').addClass('is-hidden');
          }

          const productInfo = galleryContainer.previousElementSibling;
          const moreInfoBox = productInfo.querySelector(
            '.about-project-item__info'
          );

          const productInfoVideo = productInfo.querySelector('video');

          var source = productInfoVideo.querySelector('source');

          checkSource(source, pauseButtons, muteButtons, fullscreenButtons);

          var playPromise = productInfoVideo.play();

          if (playPromise !== undefined) {
            playPromise
              .then((_) => {
                productInfoVideo.setAttribute('autoplay', 'autoplay');
                productInfoVideo.play();
              })
              .catch((error) => {
                console.log(error);
              });
      ...
      muteButtons.forEach((muteButton) => {
        muteButton.addEventListener('click', (e) => {
          ...
          const videoElem = muteButton.parentNode.parentNode.nextElementSibling.querySelector(
            '.video-banner__video'
          );
      ...
      pauseButtons.forEach((pauseButton) => {
        pauseButton.addEventListener('click', (e) => {
          e.preventDefault();
          var videoElem = pauseButton.parentNode.parentNode.nextElementSibling.querySelector(
            '.video-banner__video'
          );

          playPauseVideoHandler(pauseButton, videoElem);
        });
      });
      openInfoButtons.forEach((openInfoButton) => {
        openInfoButton.addEventListener('click', function (e) {
          e.preventDefault();
          openInfoButton.parentNode.parentNode.parentNode.classList.toggle(
            'is-active'
          );

          if (openInfoButton.textContent == 'info') {
            openInfoButton.textContent = 'close info';
          } else {
            openInfoButton.textContent = 'info';
          }
        });
        document.addEventListener('wheel', function (e) {
          if (e.deltaY > 0) {
            scrollDown(openInfoButton);
          }
        });
      });
      closeButtons.forEach((closeButton) => {
        closeButton.addEventListener('click', function (e) {
          ...
          var videoElem = closeButton.parentNode.nextElementSibling.querySelector(
            '.video-banner__video'
          );

          var playPromise = videoElem.pause();
          ...
          closeButton.parentNode.parentNode.parentNode.classList.remove(
            'is-active'
          );
      ...
      fullscreenButtons.forEach((fullscreenButton) => {
        fullscreenButton.addEventListener('click', function (e) {
          e.preventDefault();
          var videoElem = fullscreenButton.parentNode.parentNode.nextElementSibling.querySelector(
            '.video-banner__video'
          );
          console.log(videoElem);
          fullScreenVideo(videoElem, fullscreenButton);
...
jQuery(document).ready(function () {
  ...
  if (/index/.test(window.location.pathname)) {
    ...
    unitNumber.forEach((item, index) => {
      item.textContent = `NO.${index < 9 ? `o${index + 1}` : index + 1}`;
      lastUnitNumber = index + 1;
    });

    function openProduct() {
      var boxProducts = document.querySelectorAll('.project_unit');

      boxProducts.forEach((product) => {
        var boxProductContainer = product.querySelector('.gallery__container');
        var closeButton = product.querySelector('.close-li');
        var playPauseButton = product.querySelector('.pause-li');
        var muteUnmuteButton = product.querySelector('.muted-li');
        var fullscreenButton = product.querySelector('.fullscreen-li');
        var closeOpenInfoButton = product.querySelector('.info');
        boxProductContainer.addEventListener('click', function (e) {
          e.preventDefault();
          ...
          var videoElem = boxProductContainer.previousElementSibling.querySelector(
            'video'
          );
          ...
          boxProductContainer.previousElementSibling.classList.add('is-active');
        ...
        closeButton.addEventListener('click', function (e) {
          ...
          closeProject(closeButton, 'is-active');
        ...
        playPauseButton.addEventListener('click', function (e) {
          e.preventDefault();
          var video = closeButton.parentNode.nextElementSibling.querySelector(
            'video'
          );
          playPauseVideoHandler(playPauseButton, video);
        });
        ...
        closeOpenInfoButton.addEventListener('click', function (e) {
          var boxInfo = closeOpenInfoButton.parentNode.parentNode.parentNode;
          infoCloseInfoHandler(closeOpenInfoButton, boxInfo, 'is-active');
        });
        document.addEventListener('wheel', function (e) {
          if (e.deltaY > 0) {
            scrollDown(closeOpenInfoButton);
          }
        });
      });
    }
...
(ajaxurl = 'http://egofilm.puzzlestudio.eu/wp-admin/admin-ajax.php'),
  (page = 3),
  (resLength = 1);
        ...
        jQuery.ajax({
          ...
          success: function (response) {
            ...
            setTimeout(function () {
              ...
              var lenght = nProjects.filter('li').length;
              if (
                !/index/.test(window.location.pathname) &&
                !/career/.test(window.location.pathname)
              ) {
