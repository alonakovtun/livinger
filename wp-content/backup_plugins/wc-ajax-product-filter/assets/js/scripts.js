jQuery(document).ready(function($) {
	...
	wcapfFilterProducts = function() {
		...
		$.get(window.location.href, function(data) {
			...
			wcapfDropDownFilter();
			$(".product-page-filters .filter-widget ul li.chosen").parent().parent().parent().toggleClass('opened');

			var renew_pagination = $data.find('nav.woocommerce-pagination');
			console.log(renew_pagination);
			if(renew_pagination.length){
				$('nav.woocommerce-pagination').remove();
				$('ul.products').after('<nav class="woocommerce-pagination"></nav>');
				$('nav.woocommerce-pagination').append(renew_pagination.html());
			}else{
				$('nav.woocommerce-pagination').remove();
			}
