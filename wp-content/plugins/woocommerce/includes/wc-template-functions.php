...
function wc_reset_product_grid_settings()
{
  ...
  wp_cache_flush(); // Flush any caches which could impact settings or woocommerce.
}
...
if (!function_exists('woocommerce_content')) {
   ...
   * without hooks or modifying core woocommerce.
   */
...
function wc_get_gallery_image_html($attachment_id, $main_image = false)
{
  ...
  return '<div href="' . esc_url($full_src[0]) . '"  data-thumb="' . esc_url($thumbnail_src[0]) . '" data-thumb-alt="' . esc_attr($alt_text) . '" class="woocommerce-product-gallery__image swiper-slide" style="background-image:url(' . esc_url($full_src[0]) . ')"></div>';
}
// href="' . esc_url( $full_src[0] ) . '" data-fancybox="gallery"
...
if (!function_exists('woocommerce_product_subcategories')) {
  /**
   * This is a legacy function which used to check if we needed to display subcats and then output them. It was called by woocommerce.
   *
   * From 3.3 onwards this is all handled via hooks and the woocommerce_maybe_show_product_subcategories function.
   *
   * Since some woocommerce have not updated compatibility, to avoid showing incorrect categories this function has been deprecated and will
...
if (!function_exists('wc_dropdown_variation_attribute_options')) {
  ...
  function wc_dropdown_variation_attribute_options($args = array())
  {
    ...
    $html  = '<select id="' . esc_attr($id) . '" class="select-material ' . esc_attr($class) . '" name="state" data-attribute_name="attribute_' . esc_attr(sanitize_title($attribute)) . '" data-show_option_none="' . ($show_option_none ? 'yes' : 'no') . '">';
