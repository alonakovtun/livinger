...
class WC_Template_Loader {
	 ...
	 * Load a template.
	 *
	 * Handles template usage so that we can use our own woocommerce instead of the theme's.
	 *
	 * Templates are in the 'woocommerce' folder. WooCommerce looks for theme
		...
		if ( $default_file ) {
			...
			if ( ! $template || WC_TEMPLATE_DEBUG_MODE ) {
				if ( false !== strpos( $default_file, 'product_cat' ) || false !== strpos( $default_file, 'product_tag' ) ) {
					...
					$template    = WC()->plugin_path() . '/woocommerce/' . $cs_template;
				} else {
					$template = WC()->plugin_path() . '/woocommerce/' . $default_file;
				}
			}
		}
	...
	public static function comments_template_loader( $template ) {
		...
		$check_dirs = array(
			...
			trailingslashit( WC()->plugin_path() ) . 'woocommerce/',
		);
	...
	/**
	 * Force the loading of one of the single woocommerce instead of whatever template was about to be loaded.
	 *
	 * @since 3.3.0
