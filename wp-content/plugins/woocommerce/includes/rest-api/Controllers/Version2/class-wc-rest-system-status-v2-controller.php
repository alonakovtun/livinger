...
class WC_REST_System_Status_V2_Controller extends WC_REST_Controller {
				...
				'theme'              => array(
					...
					'properties'  => array(
						...
						'has_outdated_templates'  => array(
							'description' => __( 'Does this theme have outdated woocommerce?', 'woocommerce' ),
							'type'        => 'boolean',
							'context'     => array( 'view' ),
							'readonly'    => true,
						),
	...
	public function get_theme_info() {
		...
		/**
		 * Scan the theme directory for all WC woocommerce to see if our theme
		 * overrides any of them.
		 */
		$override_files     = array();
		$outdated_templates = false;
		$scan_files         = WC_Admin_Status::scan_template_files( WC()->plugin_path() . '/woocommerce/' );

		// Include *-product_<cat|tag> woocommerce for backwards compatibility.
		$scan_files[] = 'content-product_cat.php';
		$scan_files[] = 'taxonomy-product_cat.php';
		$scan_files[] = 'taxonomy-product_tag.php';

		foreach ( $scan_files as $file ) {
			$located = apply_filters( 'wc_get_template', $file, $file, array(), WC()->template_path(), WC()->plugin_path() . '/woocommerce/' );
			...
			if ( ! empty( $theme_file ) ) {
				...
				$core_version  = WC_Admin_Status::get_file_version( WC()->plugin_path() . '/woocommerce/' . $core_file );
