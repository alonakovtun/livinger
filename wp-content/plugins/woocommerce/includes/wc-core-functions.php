...
/**
 * Get template part (for woocommerce like the shop-loop).
 *
...
function wc_get_template_part( $slug, $name = '' ) {
	...
	if ( ! $template ) {
		if ( $name ) {
			...
			if ( ! $template ) {
				$fallback = WC()->plugin_path() . "/woocommerce/{$slug}-{$name}.php";
...
/**
 * Get other woocommerce (e.g. product attributes) passing attributes and including the file.
 *
...
function wc_locate_template( $template_name, $template_path = '', $default_path = '' ) {
	...
	if ( ! $default_path ) {
		$default_path = WC()->plugin_path() . '/woocommerce/';
	}
