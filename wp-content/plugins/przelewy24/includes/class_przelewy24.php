...
class Przelewy24Class {
     ...
     * @return array array(STRING Error code, STRING Token)
     * @throws Exception
     */
    public function trnRegister($redirect = false) {
        ...
        if(isset($RES["error"]) && $RES["error"] === "0" && isset($RES["token"]) && $RES["token"]) {
            $token = $RES["token"];
        } else {
            $RES["token"] = "";
            return $RES;
     ...
     * @return array array(STRING Error code, ...)
     * @throws Exception
     */
    private function callUrl($function, $ARG) {

        if(!in_array($function, array('trnRegister','trnVerify','testConnection'))) {
        ...
        foreach($ARG as $k=>$v) {
            $REQ[] = $k."=".urlencode(mb_convert_encoding($v, 'iso-8859-2', 'utf-8'));
        }
    ...
    public function availablePaymentMethods($only24at7 = true, $currency = 'PLN', $lang = 'pl')
    {
        ...
        if (!empty($res) && isset($res->error) && $res->error->errorCode === 0) {
            if (!is_array($res->result)) {
                $newResult = array();
                foreach($res->result->_->item as $elm) {
                    $newResult[] = $elm->enc_value;
                }
